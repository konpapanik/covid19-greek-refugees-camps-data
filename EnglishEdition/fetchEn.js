...
imageSeries.data = raw_data.filter((i) => i.total_confirmed_cases > 0);
            //Total All camps
            var sumcases = raw_data.reduce(
                (accumulator, currentValue) =>
                    accumulator + currentValue.total_confirmed_cases,
                0
            );
            document.getElementById("sumcasesid").innerHTML = sumcases;
            //total tests
            var sumtests = raw_data.reduce(
                (accumulator, currentValue) => accumulator + currentValue.total_samples,
                0
            );
